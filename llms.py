...
"""
        ...
        st.markdown("### ℹ️ Actual Additional Info from CSV")
        for i in reranked:
            info = df.iloc[subset_indices[i]].get("additional_info", "")
            
            if isinstance(info, str) and info.strip() and not info.strip().startswith("{"):
                st.markdown(f"**Spec {i+1}:** {info}")
